...
class RoomController extends Controller
{
    ...
    public function show(string $id, Request $request)
    {
        try {
            ...
            $room = Room::with("building")->with("roomType")->with('images')->with('assets')->find($id);
            if (!$room) {
    ...
    public function update(RoomRequest $request, $id)
    {
        ...
        if (!AdminScope::ensureBuildingAccess($admin, $room->building_id)) {
            return ApiResponse::responseJson(false, 'Bạn không có quyền chỉnh sửa phòng thuộc tòa nhà này.', 403, null, 403);
        }
    ...
    public function destroy(string $id, Request $request)
    {
        try {
            ...
            //neu la super admin thi di qua/ khong phai la admin cua toa nha thi chan lai
            if (!AdminScope::ensureBuildingAccess($admin, $room->building_id)) {
                return ApiResponse::responseJson(false, 'Bạn không có quyền truy cập vào phòng này', 403, null, 403);
            }
