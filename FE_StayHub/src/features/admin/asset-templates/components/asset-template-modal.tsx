...
interface AssetTemplateModalProps {
    isOpen: boolean;
    onClose: () => void;
    editingAssetTemplateId: number | null;
    form: {
        name: string;
...
export function AssetTemplateModal({
    isOpen,
    onClose,
    editingAssetTemplateId,
    form,
    setForm,
    onCancel,
    onSubmitSuccess,
    isFormLoading = false,
}: AssetTemplateModalProps) {
    const isEditing = editingAssetTemplateId !== null;
    ...
    const submit = async () => {
        ...
        try {
            ...
            if (isEditing && editingAssetTemplateId) {
                await updateAdminAssetTemplate(editingAssetTemplateId, payload);
            } else {
    ...
    return (
        <AnimatePresence>
            {isOpen && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center p-3 sm:p-4" role="dialog" aria-modal="true" aria-labelledby="asset-modal-title">
                    <motion.div
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                            ...
                            <div className="relative flex items-center justify-between">
                                <div>
                                    <div className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-[0.2em] text-[#f3c56b]">
                                        <Boxes className="h-3.5 w-3.5" /> Mẫu tài sản
                                    </div>
                                    <h2 id="asset-modal-title" className="mt-1 text-xl font-black tracking-tight text-[#fff4df]">
                                        {isEditing ? "Cập nhật mẫu tài sản" : "Thêm mẫu tài sản"}
                                    </h2>
                                </div>
                                <button
                                    type="button"
                                ...
                                <>
                                    ...
                                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                        <div>
                                            ...
                                            <AdminSelect
                                                ...
                                                options={unitOptions}
                                                invalid={!!errors.default_unit_name}
                                                disabled={isSaving}
                                                onChange={(nextValue) => updateForm('default_unit_name', Number(nextValue))}
                                            />
                                    ...
                                    <div>
                                        ...
                                        <textarea
                                            ...
                                            placeholder="Ghi chú cách dùng hoặc tiêu chuẩn tài sản..."
                                            disabled={isSaving}
                                        />
