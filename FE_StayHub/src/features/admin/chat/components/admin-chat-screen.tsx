...
import { Image as ImageIcon, Loader2, MessageCircle, Search, Send, Users, X, ZoomIn, ZoomOut, ChevronLeft, ChevronRight } from 'lucide-react'
...
export function AdminChatScreen() {
  ...
  return (
    <section className="h-full overflow-hidden rounded-[2rem] border border-[#3d2a18]/10 bg-[#fffaf1]/85 shadow-2xl shadow-[#6b3f1d]/10 backdrop-blur-xl">
                              ...
                              {message.attachments && message.attachments.length > 0 && (
                                message.attachments.length === 1 ? (
                                  <img
                                    ...
                                    className="mt-2 block rounded-xl border border-[#3d2a18]/15 bg-black/5 hover:opacity-95 transition cursor-pointer max-h-[420px] sm:max-h-[500px] max-w-full"
                                  />
                                ) : (
                                  <div className="mt-2 grid grid-cols-2 gap-2 max-w-md w-full">
                                    {message.attachments.map((url, i) => (
                                      <img
                                        ...
                                        className="rounded-xl border border-[#3d2a18]/15 bg-black/5 hover:opacity-95 transition cursor-pointer h-32 sm:h-40 w-full object-cover"
                                      />
                                    ))}
                                  </div>
                                )
          ...
          <div className="absolute top-6 right-6 flex items-center gap-4 z-50" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center gap-1.5 bg-white/10 backdrop-blur-md rounded-full px-4 py-1.5 text-white text-xs font-semibold select-none border border-white/10">
              <button
                ...
                className="hover:text-[#8b5e34] transition p-1"
              >
              ...
              <button
                ...
                className="ml-2 pl-2 border-l border-white/20 hover:text-[#8b5e34] transition text-[10px] font-bold text-white"
              >
                Reset
              </button>
            </div>
