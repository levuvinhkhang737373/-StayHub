...
def main():
    parser = argparse.ArgumentParser(description="Test nhanh URL iPhone/camera trước khi cấu hình vào StayHub.")
    ...
    parser.add_argument("--output", default="/tmp/stayhub-camera-test.jpg", help="File ảnh preview sẽ ghi ra")
    args = parser.parse_args()

    if is_http_stream(args.url):
        frame, used_url = load_quick_http_frame(args.url, args.username, args.password, max_seconds=8)
    else:
        used_url = args.url
        ...
        frame = load_stream_frames(payload)[-1]
