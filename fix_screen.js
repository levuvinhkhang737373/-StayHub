...
// function declarations logic - using regex to match entire block until the next block
code = code.replace(/    const roomTypeOptions = useMemo\(\(\) => mergeRoomTypeOptions\(roomTypeCatalog, form\.room_types\), \[form\.room_types, roomTypeCatalog\]\);\n/g, '');
code = code.replace(/    const assetTemplateOptions = useMemo\(\(\) => mergeAssetTemplateOptions\(assetTemplateCatalog, form\.asset_templates\), \[form\.asset_templates, assetTemplateCatalog\]\);\n/g, '');
code = code.replace(/    const findRoomTypeIndex = \(item: AdminRoomTypeResource\) => form\.room_types\.findIndex\(\(row\) => row\.source_id === item\.id \|\| row\.id === item\.id \|\| row\.name\.trim\(\)\.toLowerCase\(\) === item\.name\.trim\(\)\.toLowerCase\(\)\);\n/g, '');
code = code.replace(/    const findAssetTemplateIndex = \(item: AdminAssetTemplateResource\) => form\.asset_templates\.findIndex\(\(row\) => row\.source_id === item\.id \|\| row\.id === item\.id \|\| row\.name\.trim\(\)\.toLowerCase\(\) === item\.name\.trim\(\)\.toLowerCase\(\)\);\n/g, '');

const toggleRoomTypeRegex = /    const toggleRoomType = \(item: AdminRoomTypeResource\) => {[\s\S]*?    };\n/g;
code = code.replace(toggleRoomTypeRegex, '');
const toggleAssetTemplateRegex = /    const toggleAssetTemplate = \(item: AdminAssetTemplateResource\) => {[\s\S]*?    };\n/g;
code = code.replace(toggleAssetTemplateRegex, '');
const createQuickRoomTypeRegex = /    const createQuickRoomType = async \(\) => {[\s\S]*?    };\n/g;
code = code.replace(createQuickRoomTypeRegex, '');
const createQuickAssetTemplateRegex = /    const createQuickAssetTemplate = async \(\) => {[\s\S]*?    };\n/g;
code = code.replace(createQuickAssetTemplateRegex, '');
...
code = code.replace(/function mergeAssetTemplateOptions[\s\S]*?}\n\n/g, '');
